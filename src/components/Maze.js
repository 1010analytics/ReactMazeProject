...
const Maze = () => {
  ...
  const isLinked = (cellA, cellB) => {
    const link = cellA.links.find(
      (l) => l.row === cellB.row && l.col === cellB.col
    );
    return !!link;
  };
  ...
  const huntAndKill = () => {
    ...
    while (current) {
      const unvisitedNeighbors = getNeighbors(current).filter(
        (n) => n.links.length === 0
      );
      ...
      if (length) {
        ...
        loop: for (let row of grid) {
          for (let cell of row) {
            const visitedNeighbors = getNeighbors(cell).filter(
              (n) => n.links.length !== 0
            );
  ...
  const renderPath = (startCell, color) => {
    ...
    while (distance > 0) {
      const link = cell.links.filter(
        (l) => grid[l.row][l.col].distance === distance - 1
      )[0];
  ...
  const selectPoints = () => {
    const outermostRow = grid[grid.length - 1];
    const segmentLength = Math.floor(outermostRow.length / 4);
    const points = [];

    for (let i = 0; i < 4; i++) {
      const segmentStart = i * segmentLength;
      const indexInSegment = Math.floor(Math.random() * segmentLength);
      const cellIndex = segmentStart + indexInSegment;
      points.push(outermostRow[cellIndex]);
    }
  ...
  const positionCells = () => {
    ...
    grid.forEach((row) => {
      row.forEach((cell) => {
        ...
        cell.innerCcwX =
          Math.round(center + innerRadius * Math.cos(angleCcw)) * pixelRatio +
          lineWidth / 2;
        cell.innerCcwY =
          Math.round(center + innerRadius * Math.sin(angleCcw)) * pixelRatio +
          lineWidth / 2;
        cell.outerCcwX =
          Math.round(center + outerRadius * Math.cos(angleCcw)) * pixelRatio +
          lineWidth / 2;
        cell.outerCcwY =
          Math.round(center + outerRadius * Math.sin(angleCcw)) * pixelRatio +
          lineWidth / 2;
        cell.innerCwX =
          Math.round(center + innerRadius * Math.cos(angleCw)) * pixelRatio +
          lineWidth / 2;
        cell.innerCwY =
          Math.round(center + innerRadius * Math.sin(angleCw)) * pixelRatio +
          lineWidth / 2;
        cell.outerCwX =
          Math.round(center + outerRadius * Math.cos(angleCw)) * pixelRatio +
          lineWidth / 2;
        cell.outerCwY =
          Math.round(center + outerRadius * Math.sin(angleCw)) * pixelRatio +
          lineWidth / 2;

        const centerAngle = (angleCcw + angleCw) / 2;

        cell.centerX =
          (Math.round(center + innerRadius * Math.cos(centerAngle)) *
            pixelRatio +
            lineWidth / 2 +
            Math.round(center + outerRadius * Math.cos(centerAngle)) *
              pixelRatio +
            lineWidth / 2) /
          2;
        cell.centerY =
          (Math.round(center + innerRadius * Math.sin(centerAngle)) *
            pixelRatio +
            lineWidth / 2 +
            Math.round(center + outerRadius * Math.sin(centerAngle)) *
              pixelRatio +
            lineWidth / 2) /
          2;
      });
  ...
  return (
    <div className="maze">
      <h1>CIRCULAR MAZE PUZZLE</h1>
      <div className="btn">
        <div className="maze_btn">
          <button onClick={handleCreateMaze}>Create Maze</button>{" "}
        </div>
        <div className="maze_btn">
          <button onClick={handleSolveMaze}>Solve Maze</button>{" "}
        </div>
      </div>
